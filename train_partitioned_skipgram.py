...
if __name__ == '__main__':
    ...
    augmentation_batch_selector = np.random.choice([True,False], augmented_dataset_num_batches, p=[parameters['data_augmentation_ratio'], natural_data_ratio])
    ...
    for epoch in range(parameters['epochs']):
        ...
        syn_batch_start = 0
        syn_batch_end = parameters['batch_size']
        data_batch_start = 0
        data_batch_end = parameters['batch_size']
        ...
        if parameters['emb_partition'] is not None:
            # Construct the synonym batches separately from
            # the natural data batches
            for i, select_syn in enumerate(augmentation_batch_selector):
                if select_syn:
                    batch = syns[syns_ixs[syn_batch_start:syn_batch_end]]
                    
                    syn_batch_start += parameters['batch_size']
                    syn_batch_end += parameters['batch_size']

                    # RESTART THE INDEX IF DATASET IS EXHAUSTED
                    if syn_batch_end > num_syns:
                        syn_batch_start = 0
                        syn_batch_end = parameters['batch_size']
                else:
                    ...
                    batch = data[data_ixs[data_batch_start:data_batch_end]]
                    
                    data_batch_start += parameters['batch_size']
                    data_batch_end += parameters['batch_size']

                    # RESTART THE INDEX IF DATASET IS EXHAUSTED
                    if data_batch_end > num_syns:
                        data_batch_start = 0
                        data_batch_end = parameters['batch_size']
                ...
                batches_loss += process_word_pair_batch(
                                        focus_ixs,
                                        context_ixs,
                                        model,
                                        optimiser,
                                        sample_table,
                                        parameters['num_neg_samples'],
                                        parameters['batch_size'],
                                        phase='train',
                                        emb_partition=parameters['emb_partition'],
                                        is_syn=select_syn)
